-- TEST/UAT only: controlled two-step returns to supplier workflow.
-- The original GRN is never edited or deleted.
...
BEGIN
  ...
  IF v_constraint IS NOT NULL THEN
    EXECUTE format('ALTER TABLE public.sync_log DROP CONSTRAINT %I', v_constraint);
  END IF;
END $$;
...
ALTER TABLE public.sync_log
  ...
  CHECK (event_type IN (
    'grn_confirmed', 'materials_issued', 'production_completed', 'dispatch_delivered',
    'price_sync', 'customer_sync', 'error', 'material_variance_alert',
    'macropack_manufactured', 'reconciliation_variance_approved', 'rm_cost_updated',
    'reconciliation_completed', 'material_transfer_to_production',
    'finished_goods_transfer_to_dispatch', 'stock_take_sage_snapshot',
    'return_to_supplier_requested'
  )) NOT VALID;
...
DROP POLICY IF EXISTS "Authenticated users can read RTS" ON public.return_to_supplier_requests;
DROP POLICY IF EXISTS "Finance can create RTS" ON public.return_to_supplier_requests;
DROP POLICY IF EXISTS "Finance can update RTS" ON public.return_to_supplier_requests;
CREATE POLICY "Authenticated users can read RTS" ON public.return_to_supplier_requests
  FOR SELECT TO authenticated USING (auth.uid() IS NOT NULL);
CREATE POLICY "Finance can create RTS" ON public.return_to_supplier_requests
  FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Finance can update RTS" ON public.return_to_supplier_requests
  FOR UPDATE TO authenticated USING (auth.uid() IS NOT NULL) WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Authenticated users can read RTS items" ON public.return_to_supplier_items;
DROP POLICY IF EXISTS "Authenticated users can create RTS items" ON public.return_to_supplier_items;
CREATE POLICY "Authenticated users can read RTS items" ON public.return_to_supplier_items
  FOR SELECT TO authenticated USING (auth.uid() IS NOT NULL);
CREATE POLICY "Authenticated users can create RTS items" ON public.return_to_supplier_items
  FOR INSERT TO authenticated WITH CHECK (auth.uid() IS NOT NULL);
...
BEGIN
  ...
  SELECT * INTO v_grn FROM public.goods_received_notes WHERE id = p_grn_id FOR SHARE;
  ...
  IF NOT EXISTS (
    ...
    WHERE event_type = 'grn_confirmed' AND reference_id = p_grn_id AND status = 'success'
  ) THEN
    RAISE EXCEPTION 'The GRN must be successfully posted to Sage before an RTS can be created.';
  END IF;
  IF EXISTS (
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    WHERE id = (v_line->>'grn_item_id')::uuid AND grn_id = p_grn_id;
    IF NOT FOUND THEN RAISE EXCEPTION 'A selected return line does not belong to this GRN.'; END IF;
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.return_to_supplier_items WHERE rts_id = p_rts_id) THEN
  ...
  VALUES ('return_to_supplier_requested', p_rts_id, 'return_to_supplier', 'pending', 'RTS approved by Finance; queued for Sage.', jsonb_build_object('rtsNumber', v_rts.rts_number))
