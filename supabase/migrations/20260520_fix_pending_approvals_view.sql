-- Fix pending_approvals view - restore all entity types with updated chick schema
-- The 20260518 cleanup migration accidentally removed most entity types
...
-- Chick Purchase Orders (NEW SCHEMA - using supplier join instead of supplier_name column)
-- Status: SUBMITTED (awaiting approval)
SELECT 
  'chick_booking' as entity_type,
  cpo.id as entity_id,
  cpo.po_number as entity_number,
  COALESCE(s.name, 'Unknown Supplier') as entity_name,
...
COMMENT ON VIEW pending_approvals IS 'Unified view of all items requiring approval across the system - includes GRN, quality, production, dispatch, chick bookings, macropack, reconciliation, transfers, weigh bridge, and maintenance';
