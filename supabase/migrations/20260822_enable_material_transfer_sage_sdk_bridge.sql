-- Re-enable material transfer -> Sage SDK bridge after UAT validation.
-- The bridge worker handles this event through the protected SDK API, not direct Sage SQL.
...
BEGIN
  ...
  INSERT INTO public.sync_log (
    ...
    'material_transfers',
    'pending',
    'Material transfer accepted to Production - queued for Sage SDK RM to PD posting',
    jsonb_build_object(
...
COMMENT ON FUNCTION public.queue_sage_material_transfer()
  IS 'Queues received MES material transfers for Sage RM to PD posting through the protected SDK API.';

COMMENT ON TABLE public.material_transfers
  IS 'MES material-transfer workflow. When status becomes received, the Sage SDK bridge queues an RM to PD posting event in sync_log.';
