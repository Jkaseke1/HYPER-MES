-- Add Sales Orders and Production Tracking Tables
-- This migration adds new functionality without modifying existing tables
...
CREATE POLICY "Users can view sales orders" ON sales_orders FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "Users can insert sales orders" ON sales_orders FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update sales orders" ON sales_orders FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can view sales order items" ON sales_order_items FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "Users can insert sales order items" ON sales_order_items FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update sales order items" ON sales_order_items FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can view daily reports" ON daily_production_reports FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "Users can insert daily reports" ON daily_production_reports FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update daily reports" ON daily_production_reports FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can view production issues" ON production_issues FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "Users can insert production issues" ON production_issues FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update production issues" ON production_issues FOR UPDATE USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can view POL bookings" ON pol_bookings FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "Users can insert POL bookings" ON pol_bookings FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "Users can update POL bookings" ON pol_bookings FOR UPDATE USING (auth.uid() IS NOT NULL);
