...
|---|---|---|---|---|
| **GRN approved** (received goods from supplier) | `goods_received_notes.status='approved'`, `grn_items`, `raw_materials.current_stock` ↑, `raw_material_lots` row created (one per `grn_item`) | `grn_confirmed` | Creates the established standalone Sage **Goods Received Voucher** through the protected legacy GRV procedure. The API assigns the `HFGRV` number and verifies `DocType = 2`. | `bridge/goodsReceiptAuto.js` |
...
| **Production Issue** (issue ingredient to Production Order) | `production_order_materials.issued=true`, `raw_material_lots.qty_remaining` ↓ (FIFO), `stock_movements` (type=`consumption`), `raw_materials.current_stock` ↓ | `materials_issued` | Creates a Sage **Inventory Transaction** through the protected SDK API: `MFDR`, `Decrease`, Production (`PD`) warehouse, Sage average cost, and GL posting. The SDK transaction validates all order lines then commits or rolls back as one package. | `bridge/goodsIssueAuto.js` |
