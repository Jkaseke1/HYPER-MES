-- =====================================================
-- ADD APPROVAL WORKFLOW FIELDS TO CRITICAL TABLES
...
-- 5. RECONCILIATION PERIODS - Add review and approval tracking
ALTER TABLE reconciliation_periods 
  ADD COLUMN IF NOT EXISTS reviewed_by uuid REFERENCES profiles(id),
  ADD COLUMN IF NOT EXISTS reviewed_at timestamptz,
  ADD COLUMN IF NOT EXISTS approved_at timestamptz,
  ADD COLUMN IF NOT EXISTS variance_explanation text DEFAULT '';

-- 6. CREATE APPROVAL AUDIT LOG TABLE
...
-- 7. CREATE HELPER FUNCTION TO LOG APPROVALS
...
UNION ALL
SELECT 
  'reconciliation_periods' as table_name,
  column_name,
  data_type
FROM information_schema.columns
WHERE table_name = 'reconciliation_periods' 
  AND column_name IN ('reviewed_by', 'reviewed_at', 'approved_at', 'variance_explanation')
