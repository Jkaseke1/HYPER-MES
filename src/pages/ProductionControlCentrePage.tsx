    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <section className="rounded-xl border border-slate-200 bg-white p-5"><div className="mb-4 flex items-center gap-2"><Plus className="h-5 w-5 text-teal-600" /><h2 className="text-lg font-bold text-slate-900">Submit Digital Production Notice</h2></div>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <input type="number" min="0" placeholder="Actual output (kg)" value={form.outputQty} onChange={e => setForm({...form, outputQty:e.target.value})} className="rounded-lg border border-slate-300 px-3 py-2" />
          <input type="number" min="0" placeholder="Actual output (bags)" value={form.outputBags} onChange={e => setForm({...form, outputBags:e.target.value})} className="rounded-lg border border-slate-300 px-3 py-2" />
          <input type="number" min="0" placeholder="Rejected / damaged kg" value={form.rejectedQty} onChange={e => setForm({...form, rejectedQty:e.target.value})} className="rounded-lg border border-slate-300 px-3 py-2" />
          <input type="number" min="0" placeholder="Approved recycle kg" value={form.recycleQty} onChange={e => setForm({...form, recycleQty:e.target.value})} className="rounded-lg border border-slate-300 px-3 py-2" />
          <input placeholder="Variance reason" value={form.varianceReason} onChange={e => setForm({...form, varianceReason:e.target.value})} className="rounded-lg border border-slate-300 px-3 py-2 md:col-span-2" />
          <textarea placeholder="Declaration notes: materials, output bags, exceptions and handover" value={form.notes} onChange={e => setForm({...form, notes:e.target.value})} className="min-h-24 rounded-lg border border-slate-300 px-3 py-2 md:col-span-2" />
        </div>
