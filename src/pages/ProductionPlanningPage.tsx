...
export default function ProductionPlanningPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Production Plan' : 'New Production Plan'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Plan Number</label>
              <input value={form.plan_number} readOnly className={`${inputCls} bg-slate-50`} />
            </div>
            <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setItems([...items, emptyItem()])} className="flex items-center gap-1 text-xs font-medium text-teal-600 hover:text-teal-700">
            ...
            <div className="border border-slate-200 rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-50"><tr>
                  {['Formulation', 'Qty', 'Unit', 'Priority', 'Notes', ''].map(h => <th key={h} className="text-left px-3 py-2 font-medium text-slate-600 text-xs">{h}</th>)}
                </tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {items.map((item, idx) => (
                    <tr key={idx}>
                      <td className="px-3 py-2">
                        <select value={item.formulation_id} onChange={e => { const n = [...items]; n[idx].formulation_id = e.target.value; setItems(n); }} className="w-full px-2 py-1.5 border border-slate-300 rounded text-sm focus:outline-none focus:border-teal-500">
                          <option value="">Select</option>
                          {formulations.map(f => <option key={f.id} value={f.id}>{f.code} - {f.name}</option>)}
                        </select>
                      </td>
                      <td className="px-3 py-2"><input type="number" min={0} value={item.planned_qty} onChange={e => { const n = [...items]; n[idx].planned_qty = +e.target.value; setItems(n); }} className="w-20 px-2 py-1.5 border border-slate-300 rounded text-sm focus:outline-none focus:border-teal-500" /></td>
                      <td className="px-3 py-2"><input value={item.unit} onChange={e => { const n = [...items]; n[idx].unit = e.target.value; setItems(n); }} className="w-16 px-2 py-1.5 border border-slate-300 rounded text-sm focus:outline-none focus:border-teal-500" /></td>
                      <td className="px-3 py-2"><input type="number" min={1} value={item.priority} onChange={e => { const n = [...items]; n[idx].priority = +e.target.value; setItems(n); }} className="w-16 px-2 py-1.5 border border-slate-300 rounded text-sm focus:outline-none focus:border-teal-500" /></td>
                      <td className="px-3 py-2"><input value={item.notes} onChange={e => { const n = [...items]; n[idx].notes = e.target.value; setItems(n); }} className="w-full px-2 py-1.5 border border-slate-300 rounded text-sm focus:outline-none focus:border-teal-500" /></td>
                      <td className="px-3 py-2">{items.length > 1 && <button onClick={() => setItems(items.filter((_, i) => i !== idx))} className="text-red-400 hover:text-red-600 text-xs font-medium">Remove</button>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
