...
export default function Modal({ open, onClose, title, children, footer, size = 'md', className }: ModalProps) {
  const isFormulaEditor = className?.split(/\s+/).includes('formula-editor');
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={cn(sizeClasses[size], 'max-h-[92vh] flex flex-col p-0 overflow-hidden', className)}>
        <DialogHeader className={cn(
          'shrink-0 bg-gradient-to-r from-[#06061c] via-[#0b0c36] to-[#080829] text-white px-6 py-4 border-b border-orange-500/30',
          isFormulaEditor && 'formula-editor-modal-header',
        )}>
